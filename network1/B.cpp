...
void sendHello(){
	...
	ip.Protocol = 6;
	ip.Checksum = 0;
	ip.SourceAddr = inet_addr("211.87.236.149");
...
//	ip.DestinationAddr = inet_addr(destIP);
	...
	Result = pcap_sendpacket(adhandle2, SendBuffer, sizeof(struct ethernet_head) + sizeof(struct IpHeader) + sizeof(struct TcpHeader) + strlen(TcpData));
